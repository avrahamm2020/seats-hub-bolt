...
const ChatWidget = () => {
  ...
  useEffect(() => {
    ...
    if (!webhookUrl || typeof webhookUrl !== 'string' || webhookUrl.trim() === '') {
      setError('Webhook URL is not configured properly. Please check your .env file.');
      console.error('ChatWidget: VITE_WEBHOOK_URL is not defined or invalid:', webhookUrl);
      return;
    }
    ...
    try {
      ...
      setError('Failed to initialize chat widget. Please try refreshing the page.');
    }
  }, []);

  // Don't render anything if there's an error (fail silently)
  if (error) {
    console.warn('ChatWidget error:', error);
    return null;
  }
